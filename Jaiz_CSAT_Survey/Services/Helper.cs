...
namespace Jaiz_CSAT_Survey.Services
{
    public class Helper
    {
        public string SendNotificationEmail(string emailaddress)
        {
            ...
            try
            {
                EmailService.JaizHelperClient service = new EmailService.JaizHelperClient();
                EmailService.EmailObject obj = new EmailService.EmailObject
                {
                    Attachment = null,
                    EmailAddress = emailaddress,
                    EmailContent = strMessage,
                    FromAddress = "platform@jaizbankplc.com",
                    HasAttachment = 0,
                    SenderId = "SRVMGT",
                    Subject = "Low Survey Alert"
                };
